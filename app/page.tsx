...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-emerald-950 text-gray-800 font-sans flex flex-col justify-between pb-28">
      {/* Header */}
      <header className="bg-emerald-900 border-b border-emerald-800 text-white py-4 px-3 text-center shadow-lg">
        ...
        <p className="mt-2 text-emerald-100 text-xs font-light italic">"100% Genuine Products • Direct Delivery"</p>
        <div className="mt-2 text-[11px] bg-emerald-800/80 inline-block px-3 py-1 rounded-full text-emerald-100 border border-emerald-700">
      ...
      <main className="max-w-4xl mx-auto py-6 px-3 w-full">
        ...
        <section className="mb-6 bg-amber-50 border border-amber-200 rounded-2xl p-4 text-amber-950 shadow-md text-xs sm:text-sm">
          <p className="font-bold mb-1">💡 Simple Ordering Guide:</p>
          <ol className="list-decimal pl-5 space-y-1 text-xs">
            <li>Choose <strong>'Quick Buy'</strong> for a single item, or <strong>'Add to Cart'</strong> for multiple items.</li>
            <li>Clicking will open WhatsApp automatically with your product selection.</li>
        ...
        <section className="mb-6 bg-amber-500 border border-amber-600 rounded-2xl p-5 text-emerald-950 text-center shadow-xl">
          ...
          >
            ...
            <span>Consult Doctor via WhatsApp</span>
          </a>
        </section>
                ...
                <div key={item.id} className="bg-white rounded-2xl shadow-xl overflow-hidden border border-emerald-100 flex flex-col justify-between hover:shadow-2xl transition-all">
                  <div>
                    ...
                    <div className="p-4 text-center">
                      ...
                      <p className="text-xl font-black text-emerald-700 mb-3">{item.price}</p>
                    </div>
                  </div>
                  ...
                  <div className="p-4 pt-0 flex gap-2">
                    <button
                      ...
                      className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2.5 px-3 rounded-xl shadow-md transition-all active:scale-95 text-xs text-center"
                    >
                      Quick Buy
                    </button>
                    <button
                      onClick={() => (isSelected ? removeFromCart(item.id) : addToCart(item))}
                      className={`flex-1 text-xs font-bold py-2.5 px-3 rounded-xl border transition text-center ${
                        isSelected
      ...
      {cart.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-emerald-900 border-t border-emerald-700 shadow-2xl p-4 z-50">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="text-white">
              <p className="text-xs sm:text-sm font-bold">{cart.length} item(s) in cart</p>
              <p className="text-[10px] text-emerald-300">Ready to send order via WhatsApp</p>
            </div>
            <button
              onClick={handleCartCheckout}
              className="bg-amber-500 hover:bg-amber-600 text-emerald-950 text-xs sm:text-sm font-black px-5 py-2.5 rounded-xl shadow-lg transition-all active:scale-95"
            >
